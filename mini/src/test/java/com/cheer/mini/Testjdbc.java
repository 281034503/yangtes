...
public class Testjdbc {
	...
	public static void main(String[] args)throws Exception{
		...
		+java.util.UUID.randomUUID().toString().replaceAll("-", "")+"',"+"'张三'.22,167,55)";
   log.info(insertSql);
   stmt.execute(insertSql);
   
    insertSql="insert into sys_student(id,name,age,height,weight)values('"
			+java.util.UUID.randomUUID().toString().replaceAll("-", "")+"',"+"'李四'.22,167,55)";
log.info(insertSql);
stmt.execute(insertSql);

insertSql="insert into sys_student(id,name,age,height,weight)values('"
			+java.util.UUID.randomUUID().toString().replaceAll("-", "")+"',"+"'王五'.22,167,55)";
log.info(insertSql);
stmt.execute(insertSql);


insertSql="insert into sys_student(id,name,age,height,weight)values('"
			+java.util.UUID.randomUUID().toString().replaceAll("-", "")+"',"+"'赵六'.22,167,55)";
log.info(insertSql);
stmt.execute(insertSql);
