...
<layout:default title="资产品类列表">
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<td>
				...
				<a href="${ctx}/assets/category/form?parentId={{row.id}}">添加下级区域</a> 
			</td>
		</tr>
	</script>
	<script type="text/template" id="treeTableTplHasChild">
		<tr id="{{row.id}}" pId="{{pid}}" haschild="true">
			...
			<td>
				<a href="${ctx}/assets/category/form?id={{row.id}}">修改</a>
				<a href="${ctx}/assets/category/delete?id={{row.id}}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)">删除</a>
